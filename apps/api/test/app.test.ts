...
test("registration requires the server-issued code and limits resends", async () => {
  ...
  const withoutChallenge = await request(app)
    .post("/api/auth/register")
    .send(registration);
...
test("browser cross-site refresh is rejected without consuming the token", async () => {
  ...
  const cliRefresh = await request(app)
    .post("/api/auth/refresh")
    .set("cookie", cookie);
...
test("configuration validates ports, CORS, database URLs, secrets, and TTLs", () => {
  ...
  assert.throws(
    () => loadConfig({ ...validEnvironment, CORS_ORIGINS: "*" }),
    /explicit origins/,
  );
  assert.throws(
  ...
  assert.throws(
    () => loadConfig({ ...validEnvironment, JWT_ACCESS_SECRET: "too-short" }),
    /32/,
  );
  assert.throws(
    () => loadConfig({ ...validEnvironment, ACCESS_TOKEN_TTL: "1000" }),
    /duration/,
  );
  assert.throws(
    () => loadConfig({ ...validEnvironment, ACCESS_TOKEN_TTL: "2d" }),
    /between/,
  );
  ...
  assert.deepEqual(defaultOrigins, [
    "http://localhost:5173",
    "http://127.0.0.1:5173",
  ]);
});
