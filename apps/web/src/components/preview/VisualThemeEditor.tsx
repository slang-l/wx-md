  ...
  return (
    ...
    >
      ...
      <div className="visual-theme-navigation">
            ...
            <button
              ...
              onKeyDown={(event) => {
                ...
                if (event.key === 'ArrowRight') nextIndex = (index + 1) % editorTabs.length;
                  ...
                  nextIndex = (index + editorTabs.length - 1) % editorTabs.length;
        ...
        <span className="visual-theme-live">
      ...
      <div
        ref={panelRef}
        id={`${id}-panel`}
        className="visual-theme-editor-body"
        role="tabpanel"
          ...
          <>
              ...
              <div className="visual-theme-template-grid" role="group" aria-label="基础模板">
                {wechatThemes.map((theme) => (
                  <button
                    ...
                    onClick={() => {
                      if (active.baseThemeId !== theme.id) onBaseChange(theme.id);
                    }}
                  >
                    <span
                      className={`visual-theme-template-paper is-${theme.id}`}
                      aria-hidden="true"
                    >
                      <span className="visual-theme-template-title">一篇好文章</span>
                      <span className="visual-theme-sample-lines">
                        <i />
                        <i />
                        <i />
                      </span>
            ...
            <ControlSection title="文章配色" description="用颜色建立你的辨识度">
              <div className="visual-theme-color-card">
                ...
                <div className="visual-theme-preset-row">
                  ...
                  <div className="visual-theme-swatches" role="group" aria-label="强调色预设">
                    {accentPresets.map(({ color, label }) => (
                      ...
                      >
                        {active.settings.accentColor.toLowerCase() === color ? (
                ...
                <ColorControl
                  label="正文颜色"
                  description="舒适、清晰的阅读体验"
                  value={active.settings.textColor}
              ...
              <div className="visual-theme-control-grid">
                <label className="visual-theme-control">
                  ...
                  <span className="visual-theme-select-wrap">
                    ...
                    <select
                      value={active.settings.font}
                      onChange={(event) => update('font', event.target.value as VisualThemeFont)}
                    >
                ...
                <RangeControl
                  ...
                  step={1}
                  unit="px"
                  value={active.settings.fontSize}
            ...
            <ControlSection title="阅读节奏" description="给文字留一点呼吸的空间">
              <div className="visual-theme-control-grid">
                <RangeControl
                  ...
                  step={0.05}
                  unit="倍"
                  value={active.settings.lineHeight}
                ...
                <RangeControl
                  ...
                  step={0.1}
                  unit="em"
                  value={active.settings.paragraphSpacing}
                ...
                <RangeControl
                  ...
                  step={0.01}
                  unit="em"
                  value={active.settings.letterSpacing}
                ...
                <div className="visual-theme-control">
                  ...
                  <SegmentedControl<VisualThemeAlignment>
                    ...
                    options={[
                      ...
                      { value: 'justify', label: '两端对齐', icon: <AlignJustify size={14} /> },
                    ]}
              ...
              <label className="visual-theme-toggle">
                <span>
                  <strong>首行缩进</strong>
                  <small>每段开头缩进两个字符</small>
                </span>
                <input
                  type="checkbox"
                  role="switch"
                  checked={active.settings.firstLineIndent}
                  onChange={(event) => update('firstLineIndent', event.target.checked)}
                />
        ...
        {tab === 'elements' ? (
          <>
            <ControlSection title="标题样式" description="让内容层次一目了然">
              <StructureControl
                kind="heading"
                value={active.settings.headingStyle}
                onChange={(value) => update('headingStyle', value)}
              />
            </ControlSection>
            <ControlSection title="引用样式" description="为值得注意的文字加一点强调">
              <StructureControl
                kind="quote"
                value={active.settings.quoteStyle}
            ...
            <ControlSection title="细节与留白">
              <div className="visual-theme-control-grid">
                <RangeControl
                  ...
                  step={1}
                  unit="px"
                  value={active.settings.imageRadius}
                ...
                <RangeControl
                  ...
                  step={2}
                  unit="px"
                  value={active.settings.pagePadding}
      ...
      <footer className="visual-theme-savebar">
        ...
        <div className="visual-theme-footer-meta">
          <span className={`visual-theme-save-status ${dirty ? 'is-dirty' : ''}`} role="status">
            {dirty ? (
              <span className="visual-theme-status-dot" aria-hidden="true" />
            ) : (
              <Check size={12} aria-hidden="true" />
            )}
            {dirty ? (active.savedId ? '有未保存的更改' : '未保存的主题') : '已保存'}
            <span
              className="visual-theme-save-count"
              title={`已保存 ${savedCount} 个主题，最多 ${MAX_SAVED_VISUAL_THEMES} 个`}
            >
              {savedCount}/{MAX_SAVED_VISUAL_THEMES}
            </span>
          </span>
          <div className="visual-theme-footer-tools">
            <button type="button" onClick={onReset} title="恢复当前模板的默认样式">
              <RotateCcw size={12} />
              重置
            </button>
            {onDelete ? (
              <button
                className={`visual-theme-delete-button ${deleteArmed ? 'is-armed' : ''}`}
                type="button"
...
function ControlSection({
  ...
  return (
    <section className="visual-theme-section">
      <div className="visual-theme-section-heading">
        <h4>{title}</h4>
...
function ColorControl({
  ...
  return (
    <div className="visual-theme-color-control">
      <div className="visual-theme-color-label">
        <strong>{label}</strong>
...
function RangeControl({
  ...
  return (
    <div className="visual-theme-control visual-theme-range-control">
      ...
      />
      <div className="visual-theme-range-limits" aria-hidden="true">
        <span>{min}</span>
        <span>{max}</span>
      </div>
    </div>
...
function StructureControl({
  ...
  kind: 'heading' | 'quote';
  value: VisualThemeStructure;
  onChange: (value: VisualThemeStructure) => void;
}) {
  return (
    <div
      className="visual-theme-structure-grid"
      role="group"
      aria-label={kind === 'heading' ? '标题样式' : '引用样式'}
    >
      {(
        [
          { value: 'default', label: '经典' },
          { value: 'grace', label: '强调' },
          { value: 'simple', label: '柔和' },
        ] as const
      ).map((option) => (
        <button
          key={option.value}
          type="button"
          className={`visual-theme-structure-card ${value === option.value ? 'is-active' : ''}`}
          aria-pressed={value === option.value}
          onClick={() => onChange(option.value)}
        >
          <span
            className={`visual-theme-structure-sample is-${kind} is-${option.value}`}
            aria-hidden="true"
          >
            {kind === 'heading' ? (
              <strong>章节标题</strong>
          ...
          <span className="visual-theme-structure-caption">
            {option.label}
            <span className="visual-theme-selection-mark" aria-hidden="true">
              {value === option.value ? <Check size={10} strokeWidth={3} /> : null}
            </span>
          </span>
        </button>
      ))}
